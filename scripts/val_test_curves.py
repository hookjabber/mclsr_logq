...
for pat in sys.argv[1:]:
    for d in sorted(glob.glob('tensorboard_logs/' + pat)):
        ...
        vt = [t for t in tags if t.startswith('validation/ndcg@20')]
        et = [t for t in tags if t.startswith('eval/ndcg@20')]
        if not vt:
            print(d, 'tags:', tags[:8])
            continue
        v = {e.step: e.value for e in acc.Scalars(vt[0])}
        e = {x.step: x.value for x in acc.Scalars(et[0])} if et else {}
        print('==', d.split('/')[-1])
        for s in sorted(v):
