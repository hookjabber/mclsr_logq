...
scores[0] = -1e12
scores[-1] = -1e12  # pad / mask columns, as in the model
...
res = evaluate(
    model=PopModel(),
    dataloader=loader,
    metric_configs=inf['metrics'],
    pred_prefix=inf['pred_prefix'],
    labels_prefix=inf['labels_prefix'],
    meta=dataset.meta,
)
print(
    'MostPop via irec metrics:',
    {k: round(v, 4) for k, v in res.items() if k in ('ndcg@20', 'recall@1000', 'ndcg@50', 'recall@20', 'hit@20')},
)
...
nd, rc, n_users = [], [], 0
for batch in loader:
    lab, ln = batch['labels.ids'].numpy(), batch['labels.length'].numpy()
    off = 0
    for L in ln:
        labels = set(lab[off : off + L].tolist())
        off += L
        hits20 = [1.0 if t in labels else 0.0 for t in top[:20]]
        dcg = sum(h / np.log2(i + 2) for i, h in enumerate(hits20))
        idcg = sum(1 / np.log2(i + 2) for i in range(min(20, L)))
        nd.append(dcg / idcg if idcg > 0 else 0.0)
        rc.append(sum(1 for t in top[:1000] if t in labels) / L if L > 0 else 0.0)
        n_users += 1
