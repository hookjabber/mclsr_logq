...
parser.add_argument('--data-dir', default='data/Beauty')
...
tr_s = read('train_sasrec.txt')
tr_m = read('train_mclsr.txt')
vh, vt, th, tt = read('valid_history.txt'), read('valid_target.txt'), read('test_history.txt'), read('test_target.txt')
print(
    'lines: train_sasrec %d, train_mclsr(ladder) %d, valid %d/%d, test %d/%d'
    % (len(tr_s), len(tr_m), len(vh), len(vt), len(th), len(tt))
)
print(
    'sample train_sasrec:',
    tr_s[0][:8],
...
users_tr = {r[0] for r in tr_s}
users_v = {r[0] for r in vh}
users_t = {r[0] for r in th}
print(
    'users: train %d, valid %d, test %d | train&valid %d, train&test %d, valid&test %d'
    % (
        len(users_tr),
        len(users_v),
        len(users_t),
        len(users_tr & users_v),
        len(users_tr & users_t),
        len(users_v & users_t),
    )
)
print(
    'ladder users == train users:',
    {r[0] for r in tr_m} == users_tr,
    '| valid hist/target users equal:',
    {r[0] for r in vh} == {r[0] for r in vt},
    '| test:',
    {r[0] for r in th} == {r[0] for r in tt},
)
# q-tables
ic = pickle.load(open(D + 'item_counts.pkl', 'rb'))
tc = pickle.load(open(D + 'item_target_counts.pkl', 'rb'))
cc = pickle.load(open(D + 'item_context_counts.pkl', 'rb'))
...
ic = unwrap('item_counts', ic)
tc = unwrap('item_target_counts', tc)
cc = unwrap('item_context_counts', cc)
...
def cmp(name, table, counter):
    ...
    nz = table.sum()
    print(
        '%s: table sum %.0f, recomputed sum %.0f, max|diff| %.0f, argmax-diff item %d (table %.0f vs recomputed %.0f)'
        % (
            name,
            nz,
            v.sum(),
            np.abs(table - v).max(),
            np.abs(table - v).argmax(),
            table[np.abs(table - v).argmax()],
            v[np.abs(table - v).argmax()],
        )
    )
...
print(
    'items only in valid/test (count>0 there, 0 in train):',
    sum(1 for k in vt_items if k < len(ic) and all_items_train.get(k, 0) == 0),
    '| of them with nonzero item_counts:',
    sum(1 for k in vt_items if k < len(ic) and all_items_train.get(k, 0) == 0 and ic[k] > 0),
)
print('item id range train: %d..%d, table len %d' % (min(all_items_train), max(all_items_train), len(ic)))
# graph
g = sp.load_npz(D + 'general_graph_trainOnly.npz')
print('general_graph:', g.shape, 'nnz', g.nnz)
pairs = {(r[0], i) for r in tr_s for i in r[1:]}
print(
    'distinct train (user,item) pairs:',
    len(pairs),
    '| nnz/2 =',
    g.nnz / 2,
    '| if graph is user-item bipartite symmetric, these match',
)
for fn in ('item_graph_k50_trainOnly.npz', 'user_graph_k50_trainOnly.npz'):
    m = sp.load_npz(D + fn)
    print(fn, m.shape, 'nnz', m.nnz)
