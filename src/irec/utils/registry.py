    ...
    def __init__(cls, name, base, params, **kwargs):
        ...
        if is_base_class:
            ...
            for key in cls.mro():
                if isinstance(key, MetaParent) and key.mro()[1] is object:
                    assert (
                        base_class_found is False
                    ), 'multiple base classes(bug)'
                    base_class = key
        ...
        def __init_subclass__(scls, config_name=None):
            ...
            if config_name is not None:
                if config_name in base_class._subclasses:
                    raise ValueError(
                        'Class with name `{}` is already registered'.format(
                            config_name,
                        ),
                    )
        ...
        def parent_create_from_config(cls, config, **kwargs):
            if 'type' in config:
                ...
                raise ValueError(
                    'There is no `type` provided for the `{}` class'.format(
                        name,
                    ),
                )
        ...
        if 'create_from_config' not in cls.__dict__:
            cls.create_from_config = (
                parent_create_from_config
                if is_base_class
                else child_create_from_config
            )
